...
it("requires resource configuration permission for live discovery even in a writable session/profile editor", async () => {
  ...
  mocks.api.mockImplementation(async (_method: string, path: string) => {
    if (path === "/api/v1/universes") return [{ id: "universe", slug: "test", status: "active", role }];
  ...
  await act(async () => root.render(<QueryClientProvider client={client}><PermissionIdentityProvider userId="user" platformAdmin={false}><MemoryRouter initialEntries={["/u/test/profiles"]}><Probe /></MemoryRouter></PermissionIdentityProvider></QueryClientProvider>));
  await act(async () => { await vi.advanceTimersByTimeAsync(5); });
  expect(mocks.api.mock.calls.filter((call) => call[1].endsWith("/tools/discover"))).toHaveLength(0);
  role = "operator";
  await act(async () => { await client.invalidateQueries({ queryKey: ["universes"] }); });
  await act(async () => { await vi.advanceTimersByTimeAsync(5); });
