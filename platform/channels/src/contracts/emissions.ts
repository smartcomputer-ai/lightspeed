...
export interface WorkflowToolInvocation {
   ...
   * runtime-owned `reply` Promise even though it is hidden from the model.
   */
 ...
 * mismatch into silently lost channel work.
 */
...
/** Return the single runtime-owned reply Promise required by Channels tools. */
export function joinedReplyPromiseId(invocation: WorkflowToolInvocation): string {
  const promises = invocation.completion_promises;
  if (promises === undefined) {
    throw new TypeError(
      "pushed Joined channel invocation must have exactly the reserved `reply` completion promise",
    );
  }
  ...
  requireNonEmptyValue(promiseId, "joined reply promise");
  return promiseId;
}
