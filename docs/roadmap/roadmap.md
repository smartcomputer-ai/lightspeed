...
- [ ] [P84](p84-fleet-wait-and-callbacks.md) — `agent_wait` (terminal-only, via a generic `RunSubscription` + deferred-tool primitive) and `agent_send` (graph-shaped send along any link, replaces `agent_task` + callbacks)
- [ ] agent profiles
