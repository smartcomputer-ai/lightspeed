...
mod tests {
    ...
    use agent_core::{
        ...
        storage::{
            AppendSessionEvents, BlobEdge, BlobGraphStore, BlobStore, CreateSession,
    ...
    use crate::{PgStore, PgStoreConfig};
    ...
    #[ignore = "requires FORGE_TEST_POSTGRES_URL and creates schema tables"]
    async fn pg_store_persists_sessions_and_blobs() {
        let database_url = std::env::var("FORGE_TEST_POSTGRES_URL")
            .expect("FORGE_TEST_POSTGRES_URL must point at a disposable Postgres database");
        let config = PgStoreConfig::new(Uuid::new_v4()).with_inline_threshold_bytes(8);
        let pool = PgPoolOptions::new()
            .max_connections(1)
            .connect(&database_url)
            .await
            .expect("connect postgres");
        let store = PgStore::with_object_store(pool, Arc::new(InMemory::new()), config);
        ...
        assert_eq!(
            store.read_text(&inline_ref).await.expect("read inline"),
            "small"
        );
        assert_eq!(
            store.read_text(&object_ref).await.expect("read object"),
            "large object payload"
        );

        store
            .record_session_blob_roots(vec![SessionBlobRoot::for_seq(
                session_id,
                inline_ref.clone(),
                "event",
                EventSeq::new(1),
            ...
            .expect("record root");
        store
            .record_blob_edges(vec![BlobEdge::contains(object_ref, inline_ref)])
            .await
            .expect("record edge");
    }
}
