...
const CORE_AGENT_COMMAND_KIND: &str = "forge.core_agent.command";
...
fn is_core_agent_event_envelope_kind(kind: &str) -> bool {
    matches!(
        kind,
        "forge.core_agent.lifecycle.opened"
            | "forge.core_agent.lifecycle.config_changed"
            | "forge.core_agent.lifecycle.closed"
            | "forge.core_agent.run.queued"
            | "forge.core_agent.run.started"
            | "forge.core_agent.run.steering_added"
            | "forge.core_agent.run.cancellation_requested"
            | "forge.core_agent.run.completed"
            | "forge.core_agent.run.failed"
            | "forge.core_agent.run.cancelled"
            | "forge.core_agent.turn.started"
            | "forge.core_agent.turn.planned"
            | "forge.core_agent.turn.generation_requested"
            | "forge.core_agent.turn.generation_completed"
            | "forge.core_agent.turn.completed"
            | "forge.core_agent.context.items_recorded"
            | "forge.core_agent.context.window_planned"
            | "forge.core_agent.context.compaction_recorded"
            | "forge.core_agent.tool_config.registry_changed"
            | "forge.core_agent.tool_config.profile_selected"
            | "forge.core_agent.tool_config.default_target_set"
            | "forge.core_agent.tool_config.default_target_cleared"
            | "forge.core_agent.tool.batch_started"
            | "forge.core_agent.tool.call_started"
            | "forge.core_agent.tool.call_completed"
            | "forge.core_agent.tool.batch_completed"
    )
}
...
fn core_agent_event_envelope_kind(event: &CoreAgentEvent) -> &'static str {
    match &event.kind {
        CoreAgentEventKind::Lifecycle(event) => match event {
            CoreAgentLifecycleEvent::Opened { .. } => "forge.core_agent.lifecycle.opened",
            CoreAgentLifecycleEvent::ConfigChanged { .. } => {
                "forge.core_agent.lifecycle.config_changed"
            }
            CoreAgentLifecycleEvent::Closed => "forge.core_agent.lifecycle.closed",
        },
        CoreAgentEventKind::Run(event) => match event {
            RunEvent::Queued { .. } => "forge.core_agent.run.queued",
            RunEvent::Started { .. } => "forge.core_agent.run.started",
            RunEvent::SteeringAdded { .. } => "forge.core_agent.run.steering_added",
            RunEvent::CancellationRequested { .. } => "forge.core_agent.run.cancellation_requested",
            RunEvent::Completed { .. } => "forge.core_agent.run.completed",
            RunEvent::Failed { .. } => "forge.core_agent.run.failed",
            RunEvent::Cancelled { .. } => "forge.core_agent.run.cancelled",
        },
        CoreAgentEventKind::Turn(event) => match event {
            TurnEvent::Started { .. } => "forge.core_agent.turn.started",
            TurnEvent::Planned { .. } => "forge.core_agent.turn.planned",
            TurnEvent::GenerationRequested { .. } => "forge.core_agent.turn.generation_requested",
            TurnEvent::GenerationCompleted { .. } => "forge.core_agent.turn.generation_completed",
            TurnEvent::Completed { .. } => "forge.core_agent.turn.completed",
        },
        CoreAgentEventKind::Context(event) => match event {
            ContextEvent::ItemsRecorded { .. } => "forge.core_agent.context.items_recorded",
            ContextEvent::WindowPlanned { .. } => "forge.core_agent.context.window_planned",
        ...
        CoreAgentEventKind::ToolConfig(event) => match event {
            ...
            ToolConfigEvent::ProfileSelected { .. } => {
                "forge.core_agent.tool_config.profile_selected"
            }
        ...
        CoreAgentEventKind::Tool(event) => match event {
            ToolEvent::BatchStarted { .. } => "forge.core_agent.tool.batch_started",
            ToolEvent::CallStarted { .. } => "forge.core_agent.tool.call_started",
            ToolEvent::CallCompleted { .. } => "forge.core_agent.tool.call_completed",
            ToolEvent::BatchCompleted { .. } => "forge.core_agent.tool.batch_completed",
        },
    }
}
...
mod tests {
    ...
    fn core_agent_event_round_trips_through_dynamic_envelope() {
        ...
        assert_eq!(encoded.kind, "forge.core_agent.lifecycle.closed");
    ...
    fn core_agent_lifecycle_fixture_matches_codec() {
        assert_fixture_round_trip(
            include_str!("../../fixtures/core_agent_lifecycle_closed_dynamic_event.json"),
            CoreAgentEvent {
                kind: CoreAgentEventKind::Lifecycle(CoreAgentLifecycleEvent::Closed),
            },
        );
    ...
    fn core_agent_command_fixture_matches_codec() {
        ...
        let fixture_command = serde_json::from_str::<crate::DynamicCommand>(include_str!(
            "../../fixtures/core_agent_close_session_dynamic_command.json"
        ))
    ...
    fn core_agent_tool_fixture_matches_codec() {
        assert_fixture_round_trip(
            include_str!("../../fixtures/core_agent_tool_batch_completed_dynamic_event.json"),
            CoreAgentEvent {
    ...
    fn core_agent_dynamic_entry_fixture_matches_codec() {
        ...
        let fixture_entry = serde_json::from_str::<DynamicSessionEntry>(include_str!(
            "../../fixtures/core_agent_tool_batch_completed_dynamic_entry.json"
        ))
