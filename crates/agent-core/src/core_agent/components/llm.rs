...
use crate::{
    ...
    ResolvedContextWindow, RunId, SessionConfig, ToolChoice, ToolChoiceMode, ToolKind, ToolName,
    ToolSpec, TurnId,
};
...
pub(crate) fn build_llm_request(
    ...
    let resolved_window =
        crate::core_agent::components::context::resolve_context_window(state, window)?;
    let (tools, tool_choice) = selected_tools_and_choice(state, config, &model.api_kind)?;
    let kind = match (&model.api_kind, &config.turn.provider_request_defaults) {
