...
use agent_api::{
    AgentApiService, InputItem, ModelConfig, RunStartParams, RunStatus, SessionItemView,
...
const CASES_ROOT: &str = "crates/agent-eval/cases";
...
async fn run_attempt(
    ...
    let started = runtime
        ...
        .start_session(SessionStartParams {
            ...
            model: Some(ModelConfig {
                ...
                api_kind: oai::API_KIND.to_string(),
    ...
    let run_view = session
        .runs
        .iter()
        .find(|candidate| candidate.id == run.result.run.id)
        .unwrap_or(&run.result.run);
...
async fn build_runtime(
    ...
    let api = LocalAgentApi::builder(stores, llm_executor, default_config)
        .with_tools(tool_executor)
        .with_tool_registry(host_profile.registry)
        .with_default_tool_target(HostToolTargets::local_execution_target())
        .with_tool_profile_id(host_profile.profile_id)
        .build();
    Ok(EvalRuntime {
        api,
        tool_id_by_name,
...
fn resolve_provider(provider_id: String, model: String) -> Result<ProviderRuntime> {
    if provider_id != DEFAULT_PROVIDER_ID {
        bail!("agent-eval currently supports only --provider openai");
    }
    let api_key =
...
mod tests {
        ...
        let observations = collect_observations(
            &[
                ...
                SessionItemView::ToolResult {
                    ...
                    is_error: false,
                    status: agent_api::ToolItemStatus::Succeeded,
                },
    ...
    fn host_tool_for_id_resolves_supported_tools() {
        ...
        assert_eq!(
            tool.name(&agent_tools::runtime::ToolTarget::from(&ModelSelection {
                api_kind: ProviderApiKind::OpenAiResponses,
                provider_id: "openai".into(),
                model: "gpt-5.5".into(),
                options: ModelProviderOptions::None,
            })),
            agent_core::ToolName::new("grep")
        );
    }
}
