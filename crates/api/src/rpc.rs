...
api_methods! {
    ...
    METHOD_SESSION_START => start_session(SessionStartParams) -> SessionStartResponse =>
        ["Create or reopen a session", "Creates a session with optional config/profile setup. Profile metadata and retention supply creation defaults; explicit start values override them. The default environment attachment in the effective config supplies the initial active environment. Retrying an existing session id returns that session."], access: MethodAccess::Universe(UniverseAction::CreateSession),
    ...
    METHOD_SESSION_LIST => list_sessions(SessionListParams) -> SessionListResponse =>
        ["List sessions", "Returns a cursor-paginated summary list ordered by most recent update. Pages may shift while sessions are changing."], access: MethodAccess::Universe(UniverseAction::Read),
    ...
    METHOD_SESSION_ENVIRONMENTS_ACTIVATE => activate_session_environment(SessionEnvironmentActivateParams) -> SessionEnvironmentActivateResponse =>
        ["Activate a session environment", "Selects an allowed, live universe environment for environment-targeted tools while the session is idle."], access: MethodAccess::Universe(UniverseAction::ControlSession),
    ...
    METHOD_ENVIRONMENTS_CREDENTIALS_BIND => bind_environment_credential(EnvironmentCredentialBindParams) -> EnvironmentCredentialBindResponse =>
        ["Bind a credential into an environment", "Maps an environment variable name to an existing grant/provider/direct-secret handle for a universe environment. The response exposes only the source handle, never secret material."], access: MethodAccess::Universe(UniverseAction::ConfigureResource),
    ...
    METHOD_ENVIRONMENTS_CREATE => create_environment(EnvironmentCreateParams) -> EnvironmentCreateResponse =>
        ["Create an environment", "Records an idempotent provisioning intent against an enabled universe binding. The provider validates its provider-wide template and provisions through its backend asynchronously."], access: MethodAccess::Universe(UniverseAction::ConfigureResource),
    ...
    METHOD_ENVIRONMENTS_LIST => list_environments(EnvironmentListParams) -> EnvironmentListResponse =>
        ["List environments", "Lists universe-owned environment resources, optionally filtered by provider, binding, or logical lifecycle state."], access: MethodAccess::Universe(UniverseAction::Read),
    ...
    METHOD_ENVIRONMENTS_EXTERNAL_CREATE => create_external_environment(EnvironmentExternalCreateParams) -> EnvironmentExternalCreateResponse =>
        ["Register an external environment", "Creates an environment backed by a Lightspeed-reachable envd WebSocket endpoint. Reachability is checked on demand."], access: MethodAccess::Universe(UniverseAction::ConfigureResource),
    ...
    METHOD_BLOBS_READ => read_blob(BlobReadParams) -> BlobReadResponse =>
        ["Read a content-addressed blob", "Returns the complete immutable blob as base64; large values count against gateway and MCP response limits."], access: MethodAccess::Universe(UniverseAction::Read),
    ...
    METHOD_VFS_WORKSPACES_CREATE => create_vfs_workspace(VfsWorkspaceCreateParams) -> VfsWorkspaceCreateResponse =>
        ["Create a mutable VFS workspace", "Creates a universe workspace at an optional seed snapshot; absence starts from a server-created empty snapshot."], access: MethodAccess::Universe(UniverseAction::ConfigureResource),
    ...
    METHOD_VFS_WORKSPACES_LIST => list_vfs_workspaces(VfsWorkspaceListParams) -> VfsWorkspaceListResponse =>
        ["List VFS workspaces", "Lists mutable universe workspaces with head snapshots, sizes, and revisions."], access: MethodAccess::Universe(UniverseAction::Read),
    METHOD_VFS_WORKSPACES_UPDATE => update_vfs_workspace(VfsWorkspaceUpdateParams) -> VfsWorkspaceUpdateResponse =>
        ["Update a VFS workspace", "Moves the workspace head to an existing snapshot and updates its display name. Pass expectedRevision from a read to prevent lost updates."], access: MethodAccess::Universe(UniverseAction::ConfigureResource),
    ...
    METHOD_MCP_SERVERS_PUT => put_mcp_server(McpServerPutParams) -> McpServerPutResponse =>
        ["Create or replace an MCP server record", "Stores the complete universe catalog document, including its optional universe auth-grant credential. Use expectedRevision when replacing; token material is never accepted or returned."], access: MethodAccess::Universe(UniverseAction::ConfigureResource),
    ...
    METHOD_MCP_SERVERS_LIST => list_mcp_servers(McpServerListParams) -> McpServerListResponse =>
        ["List MCP server records", "Lists universe catalog entries, optionally filtered by lifecycle/configuration status."], access: MethodAccess::Universe(UniverseAction::Read),
    ...
    METHOD_AUTH_GRANTS_LEASE => lease_auth_grant(AuthGrantLeaseParams) -> AuthGrantLeaseResponse =>
        ["Lease a retrievable authentication grant", "Service callers only. Resolves the current access token through the broker, records the lease, and returns it once. Cache only in memory until expiry minus margin (or at most five minutes without expiry), re-lease after target 401/403, and never persist or place the token in workflow payloads."], access: MethodAccess::Service(ServiceCapability::LeaseCredentials),
    METHOD_AUTH_GRANTS_READ => read_auth_grant(AuthGrantReadParams) -> AuthGrantReadResponse =>
        ["Read authentication grant metadata", "Returns principal, provider binding, scopes, audience, expiry, status, and token-presence flags; access and refresh token values are never returned."], access: MethodAccess::Universe(UniverseAction::Read),
    ...
    METHOD_BOTS_LIST => list_bots(BotListParams) -> BotListResponse =>
        ["List bots", "Returns the roster: every bot with its trigger count, pending event count, and latest event."], access: MethodAccess::Universe(UniverseAction::Read),
    ...
    METHOD_BOTS_TRIGGERS_READ => read_bot_trigger(BotTriggerReadParams) -> BotTriggerReadResponse =>
        ["Read a trigger", "Returns one trigger with its incidents and cursor; the ingest path and pairing code are shown only to managing principals."], access: MethodAccess::Universe(UniverseAction::Read),
    METHOD_BOTS_TRIGGERS_LIST => list_bot_triggers(BotTriggerListParams) -> BotTriggerListResponse =>
        ["List a bot's triggers", "Returns every trigger of the bot ordered by id, secrets redacted for non-managing principals."], access: MethodAccess::Universe(UniverseAction::Read),
