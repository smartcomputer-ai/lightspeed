...
use api::{
    AgentApiError, BlobHasItem, BlobHasManyParams, BlobPutManyParams, BlobPutParams,
    BlobPutResponse, VfsSnapshotCommitParams, VfsSnapshotCommitResponse,
};
...
use serde::Serialize;
use vfs::{
    VfsPath, VfsSnapshotLimits, VfsSnapshotManifest, create_manifest_directory,
...
mod tests {
    ...
    use api::{BlobHasItem, BlobPutResponse, VfsSnapshotCommitResponse};
    ...
    impl CasVfsApi for FakeCasVfsApi {
        ...
        async fn commit_vfs_snapshot(
            ...
            Ok(VfsSnapshotCommitResponse {
                snapshot_ref: BlobRef::from_bytes(&bytes).as_str().to_owned(),
                files: manifest.totals.files,
                bytes: manifest.totals.bytes,
            })
        }
