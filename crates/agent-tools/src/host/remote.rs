    ...
    T: JsonRpcTransport + Send + 'static,
{
    ...
    pub fn into_tool_context(self, blobs: Arc<dyn BlobStore>) -> HostToolContext {
        let fs = Arc::new(self.file_system());
        let process = self
            .process_executor()
            .map(|executor| Arc::new(executor) as Arc<dyn ProcessExecutor>);
        let mut ctx = HostToolContext::new(fs, process, blobs);
...
mod tests {
    ...
    use super::*;
    use crate::host::tools::{ReadFileArgs, WriteFileArgs, invoke_read_file, invoke_write_file};
    ...
    #[tokio::test(flavor = "current_thread")]
    async fn remote_connection_builds_host_tool_context() {
        ...
        let ctx = connection.into_tool_context(Arc::new(InMemoryBlobStore::new()));

        assert_eq!(ctx.cwd, Some(FsPath::new("/workspace").expect("cwd")));
        assert!(ctx.process.is_none());
        assert_eq!(ctx.fs.access_policy(), FileAccessPolicy::FullReadOnly);
    }
