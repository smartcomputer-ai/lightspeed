...
mod tests {
    ...
    fn call_with_target(
        ...
        tool_name: &str,
        execution_target: Option<agent_core::ToolExecutionTarget>,
    ...
    async fn inline_runtime_maps_tool_name_to_host_operation() {
        ...
        let profile = resolve_host_profile(
            &ctx,
            &ToolTarget::api_kind(agent_core::ProviderApiKind::OpenAiResponses),
    ...
    async fn inline_runtime_maps_claude_code_like_tool_arguments() {
        ...
        let profile = resolve_host_profile(
            &ctx,
            &ToolTarget::api_kind(agent_core::ProviderApiKind::AnthropicMessages),
    ...
    #[tokio::test(flavor = "current_thread")]
    async fn core_agent_tools_reads_arguments_and_writes_result_blobs() {
        let blobs = Arc::new(InMemoryBlobStore::new());
        let fs = InMemoryFileSystem::full_access();
        fs.write_file(&FsPath::new("/file.txt").expect("path"), b"hello".to_vec())
            .await
    ...
    #[tokio::test(flavor = "current_thread")]
    async fn core_agent_tools_invokes_batch_and_writes_result_blobs() {
        let blobs = Arc::new(InMemoryBlobStore::new());
        let fs = InMemoryFileSystem::full_access();
        fs.write_file(&FsPath::new("/file.txt").expect("path"), b"hello".to_vec())
            .await
        ...
        let result = CoreAgentTools::invoke_batch(
            ...
            ToolInvocationBatchRequest {
                ...
                turn_id: TurnId::new(1),
                batch_id: ToolBatchId::new(1),
                calls: vec![agent_core::ToolInvocationRequest {
                    call_id: ToolCallId::new("call-1"),
    ...
    #[tokio::test(flavor = "current_thread")]
    async fn core_agent_tools_resolves_host_target_id_to_context() {
        let blobs = Arc::new(InMemoryBlobStore::new());
    ...
    #[tokio::test(flavor = "current_thread")]
    async fn core_agent_tools_requires_host_execution_target() {
        let blobs = Arc::new(InMemoryBlobStore::new());
        let fs = InMemoryFileSystem::full_access();
        let ctx = HostToolContext::new(Arc::new(fs), None, blobs.clone());
        let profile = resolve_host_profile(
    ...
    #[tokio::test(flavor = "current_thread")]
    async fn core_agent_tools_rejects_non_host_and_unknown_targets() {
        let blobs = Arc::new(InMemoryBlobStore::new());
        let fs = InMemoryFileSystem::full_access();
        let ctx = HostToolContext::new(Arc::new(fs), None, blobs.clone());
        let profile = resolve_host_profile(
        ...
        let cases = [
            (
                agent_core::ToolExecutionTarget::new("connector", "local"),
