//! Claude Code-like host tool surface.
...
use super::{
    EditFileArgs, GlobArgs, GrepArgs, GrepResult, HostToolOperation, RunProcessArgs, WriteFileArgs,
    invoke_edit_file, invoke_glob, invoke_grep, invoke_read_file, invoke_run_process,
    invoke_write_file,
    shared::{
...
pub(super) fn description(operation: HostToolOperation, scoped_paths: bool) -> ToolResult<String> {
    ...
    let text = match operation {
        HostToolOperation::ReadFile => "Reads a file from the filesystem.",
        HostToolOperation::WriteFile => "Writes a file to the filesystem.",
        HostToolOperation::EditFile => "Performs exact string replacements in a file.",
        HostToolOperation::Grep => "Searches file contents with a regular expression.",
        HostToolOperation::Glob => "Finds files by glob pattern.",
        HostToolOperation::RunProcess => "Executes a shell command.",
        HostToolOperation::ApplyPatch
        | HostToolOperation::ListDir
        | HostToolOperation::WriteProcessStdin => return Err(unsupported(operation)),
    };
    Ok(format!("{text}{path_guidance}"))
}

pub(super) fn input_schema(operation: HostToolOperation) -> ToolResult<Value> {
    let schema = match operation {
        HostToolOperation::ReadFile => object(
            [
                (
                    "file_path",
        ...
        HostToolOperation::WriteFile => object(
            [
                (
                    "file_path",
        ...
        HostToolOperation::EditFile => object(
            [
                (
                    "file_path",
        ...
        HostToolOperation::RunProcess => object(
            [
...
pub(super) async fn invoke_json(
    ...
    match operation {
        HostToolOperation::ReadFile => {
            ...
            let result = invoke_read_file(ctx, args.try_into_read_file_args()?).await?;
        ...
        HostToolOperation::WriteFile => {
            ...
            let result = invoke_write_file(ctx, args.try_into_write_file_args()?).await?;
            let visible = format!(
                "Wrote {} bytes to {}",
        ...
        HostToolOperation::EditFile => {
            ...
            let result = invoke_edit_file(ctx, args.try_into_edit_file_args()?).await?;
            let visible = format!(
        ...
        HostToolOperation::Grep => {
            ...
            let result = invoke_grep(ctx, args.try_into_grep_args()?).await?;
        ...
        HostToolOperation::Glob => {
            ...
            let result = invoke_glob(ctx, args.try_into_glob_args()?).await?;
            let visible = result
                .matches
                .iter()
        ...
        HostToolOperation::RunProcess => {
            ...
            let result = invoke_run_process(ctx, args.into_run_process_args()).await?;
            let visible = process_visible_output(&result);
            encode_output(&result, visible)
        }
        HostToolOperation::ApplyPatch
        | HostToolOperation::ListDir
        | HostToolOperation::WriteProcessStdin => Err(unsupported(operation)),
    }
}

fn unsupported(operation: HostToolOperation) -> ToolError {
    ToolError::UnsupportedCapability {
        message: format!(
            "ClaudeCodeLike host tool surface does not support {}",
...
impl ClaudeCodeReadArgs {
    fn try_into_read_file_args(self) -> ToolResult<super::ReadFileArgs> {
        Ok(super::ReadFileArgs {
